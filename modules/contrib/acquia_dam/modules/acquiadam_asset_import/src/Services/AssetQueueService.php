...
class AssetQueueService implements AssetQueueInterface {
  ...
  private function addAssetsToQueueBySource(string $source, array $import_list): ?int {
    ...
    foreach ($import_list as $source_uuid => $bundles) {
      ...
      switch ($source) {
        case 'categories':
          $response = $this->fetchAssets($source, $source_uuid, $this->getFilters($bundles));
          break;
        case 'asset_groups':
          $response = $this->fetchAssets($source, $source_uuid, $this->getFilters($bundles));
          break;
  ...
  /**
   * Fetch assets in a category.
   *
   * @param string $type
   *   The category or asset group.
   * @param string $uuid
   *   The list UUID.
   ...
   */
  private function fetchAssets(string $type, string $uuid, string $filters = ''): ?array {
    try {
      $response = [];
      switch ($type) {
        case 'categories':
          $response = $this->userClientFactory->getSiteClient()->getAssetsInCategory($uuid, $filters);
          break;
        case 'asset_groups':
          $response = $this->userClientFactory->getSiteClient()->getAssetsInAssetGroup($uuid, $filters);
          break;
      }
