...
class GlExchangeAdapter {
   ...
   */
  public function __construct(EntityTypeManagerInterface $entity_type_manager, ModuleHandlerInterface $module_handler_interface, ModuleExtensionList $extension_list) {
  ...
  public function getGlExchange(\PDConfig $pd_config) {
    $exchange = new \GLExchange($pd_config);
    $exchange->setAdaptorName('Drupal');
    $exchange->setAdaptorVersion(\Drupal::VERSION);
    $info = $this->extensionList->getExtensionInfo('globallink');
    $exchange->setClientVersion($info['version'] ?? 'dev');
    return $exchange;
  }
  ...
  /**
   * Get the file formats for the project
   */
