...
class GlobalLinkTranslatorUi extends TranslatorPluginUiBase {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['pd_notify_emails'] = [
      ...
      '#description' => t('A comma separated list of emails to notify. Leave blank for no notifications'),
    ];
    $form['pd_combine'] = [
      '#type' => 'checkbox',
  ...
  public function validateConfigurationForm(array &$form, FormStateInterface $form_state) {
    ...
    try {
      ...
      for($i=0; $i < count($projectIdList); $i++){
          $currentProject = $glexchange->getProject($projectIdList[$i]);
      ...
      if($duplicate){
          $form_state->setError( $settings, t('No duplicate project IDs are allowed.'));
      }
      elseif(!in_array($classifier, $classifierList)){
          $form_state->setError( $settings, t('You have selected a classifier that is not available in the PD project. Please talk to your admin to get the correct classifier.'));
      }
      else {
  ...
  public function checkoutSettingsForm(array $form, FormStateInterface $form_state, JobInterface $job) {
    ...
    if ($job->isContinuous()) {
      ...
      $form['due'] = [
        ...
        '#description' => t('Due will be set using %timezone timezone.', ['%timezone' => date_default_timezone_get()]),
      ];
  ...
  public function addfilterSubmit(array $form, FormStateInterface $form_state) {
    ...
    $this->messenger()->addStatus(t('Filter added.'));
  }
