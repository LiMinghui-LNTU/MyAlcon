...
// BC for versions of TMGMT without the MultipleCheckoutInterface.
if (!interface_exists(MultipleCheckoutInterface::class)) {
  interface BCMultipleCheckoutInterface { }
  class_alias(BCMultipleCheckoutInterface::class, MultipleCheckoutInterface::class);
}

use Drupal\user\Entity\User;
use Drupal\tmgmt\MultipleCheckoutInterface;
 ...
 * )
 */
class GlobalLinkTranslator extends TranslatorPluginBase implements ContainerFactoryPluginInterface, ContinuousTranslatorInterface, MultipleCheckoutInterface {
   ...
   * @var bool
   */
  protected bool $startSubmission = TRUE;

  /**
   * Uploaded job items waiting for submission.
   *
   * @var array
   */
  protected array $uploadedJobItems = [];

  /**
   * The active GLExchange session.
   *
   * @var \GLExchange|NULL
   */
  protected ?\GLExchange $activeGLExchange = NULL;
  ...
  public function requestTranslation(JobInterface $job) {
    $this->requestJobItemsTranslation($job->getItems(), $job);
    if (!$job->isRejected()) {
  ...
  public function requestTranslationMultiple(array $jobs) {
    ...
    while ($jobs) {
      ...
      $this->doRequestTranslations($job->getItems(), $job);
      if (!$job->isRejected()) {
        $job->submitted('The translation job has been submitted.');
      }
    }
  ...
  public function retrieveTranslation($ticket_id, JobInterface $job) {
    ...
    try {
      ...
      if ($xml === FALSE) {
        throw new \UnexpectedValueException($this->getLibxmlErrorMessage($data));
      }
  ...
  public function fetchJobs(Job $job) {
    ...
    if (!$translations_imported) {
      $this->messenger()->addStatus(t('No translations available for this job at the moment.'));
    }
  }
  ...
  public function requestJobItemsTranslation(array $job_items) {
    $this->doRequestTranslations($job_items, reset($job_items)->getJob());
  }

  /**
   * Request translation from GlobalLink.
   *
   * @param array $job_items
   *   Job items to be processed.
   * @param \Drupal\tmgmt\JobInterface $job
   *   The job to which the items belong.
   *
   * @return void
   */
  protected function doRequestTranslations(array $job_items, JobInterface $job) {
    try {
      ...
      if (!$this->activeGLExchange) {
        // @todo Not sure if we should prevent multiple submission.
        if ($job->isContinuous()) {
        ...
        if($glexchange->isSubmitterValid($settings['pd_projectid'], $userName)){
          $submission->submitter = $userName;
        }
        $glexchange->initSubmission($submission);
        $this->activeGLExchange = $glexchange;
      }
      ...
      if ($job->getTranslator()->getSetting('pd_combine')) {
        ...
        $upload_result_id =  $this->activeGLExchange->uploadTranslatable($document);
        foreach ($job_items as $job_item) {
          $this->uploadedJobItems[(string) $upload_result_id] = $job_item;
        }
      }
      else {
        foreach ($job_items as $job_item) {
      ...
      if ($this->startSubmission) {
        $submission_result_id =  $this->activeGLExchange->startSubmission();

        /** @var JobItemInterface $job_item */
        foreach ($this->uploadedJobItems as $remote_result_id => $job_item) {
          $job_item->addRemoteMapping(NULL, $submission_result_id, ['remote_identifier_2' => $remote_result_id]);
  ...
  /**
   * Formats LibXML error message to string.
   *
   * @param string $data
   *   Raw XML data.
   * @param \libXMLError|null $error
   *   The libXML Error to format. The last error is used by default.
   *
   * @return string
   *   The error message.
   */
  protected function getLibxmlErrorMessage($data, \libXMLError $error = NULL) {
    if (!isset($error)) {
    ...
    // @see https://www.php.net/manual/ru/function.libxml-get-errors.php#refsect1-function.libxml-get-errors-examples
    $strings = explode("\n", $data);
    $string = $strings[$error->line - 1];
    if (mb_strlen($string) > 255) {
      $string = mb_substr($string, 0, 252) . '...';
    }

    return sprintf('XML parse error "%s" on line %d column %d "%s".', trim($error->message), $error->line, $error->column, $string);
  }
