...
class Filterhtmlawed extends FilterBase {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $form['config'] = array(
      ...
      '#description' => $this->t("Comma-separated, quoted key-value pairs for htmLawed's %config parameter (beware of security vulnerability arising from values even if 'safe' is set to 1)", array('%config' => 'config')),
    );
    $form['spec'] = array(
      '#type' => 'textarea',
      '#rows' => '2',
  ...
  public function tips($long = FALSE) {
    ...
    $help = !empty($help) ? $help : (!$long ? $this->t('HTML markup is restricted/corrected with the htmLawed filter.') : $this->t('HTML markup is restricted/corrected with the <a href=":htmLawed">htmLawed</a> filter for compliance with admin. policy and standards and for security. More details about the restrictions in effect may be available elsewhere, such as in the text of the filter-tips of text formats that use htmLawed and on the forms for configuring text formats.', array(':htmLawed' => 'http://www.bioinformatics.org/phplabware/internal_utilities/htmLawed')) . (!\Drupal::currentUser()->hasPermission('administer filters') ? '' : $this->t(' For information on configuring the htmLawed filter, visit the htmLawed module <a href=":help">help</a> section.', array(':help' => Url::fromUri('base:admin/help/htmlawed')->toString()))));
    return $help;
  }
}
